...
export class AiFactoryBridgeClient {
  ...
  async listQueued(): Promise<AiFactoryQueuedPacket[]> {
    const payload = await this.request<AiFactoryQueueResponse>("/api/ghostwriter-bridge/queue", { method: "GET" });
  ...
  async markConsumed(bridgeId: number): Promise<void> {
    ...
    await this.request(`/api/ghostwriter-bridge/${bridgeId}/consume`, { method: "POST" });
  }
  ...
  async pullInto(
    ...
    const packets = queued.slice(0, clampLimit(options.limit));
...
function validatePacket(value: unknown): AiFactoryQueuedPacket {
  if (!value || typeof value !== "object") throw new Error("AI Factory bridge returned a non-object packet");
...
function normalizeBaseUrl(value: string): string {
  ...
  catch { throw new Error("AI_FACTORY_URL must be an absolute URL"); }
